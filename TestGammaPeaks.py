...
mean_rec.setConstant()
recoil_smearing = RooGaussian('recoil_smearing','recoil_smearing',rec,mean_rec,sigma_rec)
rec.setBins(10000,'fft')

# position of bands
ER_centroid_func = RooFormulaVar('ER_centroid_func','@0*(1+(0.16*@0^0.18)*(@1/3))/(1+@1/3)',RooArgList(rec,voltage))

# gaussians in ionization energy with shifting mean in recoil energy
ion_gauss_ER = RooGaussian('ion_gauss_ER','gauss with shifted mean',ion,ER_centroid_func,sigma_ion)
...
gamma_bckgd_pdf = RooAddPdf('gamma_bckgd','gamma_bckgd',RooArgList(Cr51_peak_pdf, Fe55_peak_pdf, Co56_peak_pdf, Zn65_peak_pdf, Ge68_peak_pdf, Ge68_2_peak_pdf),RooArgList(Cr51_peak_coeff, Fe55_peak_coeff, Co56_peak_coeff, Zn65_peak_coeff, Ge68_peak_coeff),kTRUE)
...
#gamma_bckgd_pdf = RooProdPdf('gamma_bckgd_pdf','gamma_bckgd_pdf',RooArgList(gamma_bckgd_ion, efficiency_pdf))
...
#gamma_bckgd_pdf = gamma_bckgd_rec
