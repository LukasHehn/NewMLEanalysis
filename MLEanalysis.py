...
def PerformMLE(mass_of_wimp, MC_Simulations):
  ...
  # fit parameter
  this['ratio'] = RooRealVar('ratio_%sGeV'%mass_of_wimp,'ratio_%sGeV'%mass_of_wimp,0,-1,1)
  ...
  this['f_norm'] = wm.get_normalization().getVal()
  ...
  # background
  this['gamma_pdf'] = RooProdPdf('gamma_pdf_%sGeV'%mass_of_wimp,'gamma_pdf',this['ion_gauss_ER'],this['gamma_spectrum'])
  this['gamma_pdf_smeared'] = RooFFTConvPdf('gamma_pdf_smeared_%sGeV'%mass_of_wimp,'gamma pdf * heat gaussian',rec,this['gamma_pdf'],this['heat_smearing'],2)
  ...
  this['wimp_spectrum_smeared'].setBufferFraction(0.7)
  this['final_wimp_pdf'] = RooProdPdf('final_wimp_pdf_%sGeV'%mass_of_wimp,'final_wimp_pdf',this['ion_gauss_NR'],this['wimp_spectrum_smeared'])
  ...
  result['cross section limit'] = this['f_norm'] * (result['event limit']/result['N_signal'])
  return True
...
def PlotFitResults(mass_of_wimp, toydata):
  ...
  paramframe = this['MC_study'].plotParam(this['ratio'],RooFit.FrameRange(-0.1,0.1),RooFit.Binning(200))
...
def PlotExclusionCurve():
  graph_Eric = TGraph(len(EricsResults[DetectorName]))
  graph_MLE = TGraph(len(Results))
  graph_Eric.SetTitle('Exclusion Plot %' % DetectorName)

  i = 0
  for mass_of_wimp in EricsResults[DetectorName]:
    graph_Eric.SetPoint(i, mass_of_wimp, EricsResults[DetectorName][mass_of_wimp])
    i += 1

  j = 0
  for mass_of_wimp in Results:
    graph_MLE.SetPoint(j, mass_of_wimp, Results[mass_of_wimp]['cross section limit'])
    j += 1

  canvas = TCanvas('canvas','Exclusion Plot',800,600)
  gPad.SetLogy()
  ...
  graph1.GetXaxis().SetTitle('m_\chi [GeV]')
...
# list of WIMP masses
WIMP_Masses = [5,6,7,8,9,10,12,15,20]
